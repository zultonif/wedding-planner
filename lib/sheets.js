...
export async function appendRow(sheetName, values) {
  ...
  // Gunakan range eksplisit kolom A:G (bukan hanya nama sheet) agar Google Sheets
  // tidak salah deteksi blok data dan menulis ke kolom yang jauh.
  // Hitung kolom akhir berdasarkan jumlah nilai (ID + values).
  const totalCols = 1 + values.length; // kolom A = ID, lalu values
  const lastColLetter = String.fromCharCode(64 + totalCols); // A=65, B=66, dst.
  const explicitRange = `${sheetName}!A:${lastColLetter}`;
